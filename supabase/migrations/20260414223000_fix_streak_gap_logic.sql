-- Fix streak progression: weekly shield can cover exactly one missed day only.
-- If the gap is 2+ missed days (date diff >= 3), streak must reset to 1.
...
BEGIN
  ...
  ELSIF (v_today_jst - v_last_studied_date) = 2 AND v_weekly_shield_count > 0 THEN
    v_current_streak_days := v_current_streak_days + 1;
