...
- `npm run cf:preview` の前に、`env.cloudflare-local` のローカル D1 へ `npm run d1:migrate:local` で migration を適用する。
- remote staging では `npx wrangler d1 migrations apply quizly-staging --env staging --remote` で D1 migration を適用してから `quizly-staging` Worker を deploy する。
- 環境ごとの役割は [`docs/environments.md`](environments.md) を参照する。
