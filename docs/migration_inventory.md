...
| Auth/session | App owner | `docs/auth_spec.md`, `src/lib/auth/*`, `src/middleware.ts` |
...
現行本番は `main` の Vercel/Supabase/Upstash 構成で独立稼働させる。Cloudflare 移行は `codex/cloudflare-migration` の長命ブランチと `https://quizly.stdy.workers.dev/` 上で、現行本番に影響しない完全分離環境として育てる。

この方針により、Cloudflare 側へ「いずれ消す予定の Supabase/Upstash secrets」を暫定追加しない。既存依存は migration branch 内で Clerk/D1/KV/DO/R2 へ置換し、置換が完了するまで `main` へ混ぜない。

| Issue | 分離方針 | main への扱い |
|---|---|---|
| #25 | 完了済み。移行棚卸しと freeze ルールは main に反映済み | 反映済み |
| #26 | Cloudflare runtime / Workers deploy 経路を migration branch に作る | cutover まで merge しない |
| #27 | Clerk 認証へ置換し、Supabase Auth 依存を migration branch 内で外す | cutover まで merge しない |
| #28 | D1 schema を migration branch に追加し、Supabase Postgres から独立させる | cutover まで merge しない |
| #29 | Supabase RPC を app/service layer + D1 transaction へ置換する | cutover まで merge しない |
| #30 | RLS 相当の app-layer guard を migration branch 上で完成させる | cutover まで merge しない |
| #31 | Upstash Redis を KV/Durable Objects へ置換する | cutover まで merge しない |
| #32 | Supabase Storage を R2 へ置換する | cutover まで merge しない |
| #33 | 分離環境で full dry-run/rehearsal を実行し、検証証跡を残す | 本番 cutover 判断までは merge しない |
| #34 | maintenance window で production cutover を実行する。ここが migration branch を本番へ昇格する境界 | cutover 手順として merge/deploy |
| #35 | cutover 後の production verification と rollback close | cutover 後に main/prod 前提で実施 |

例外として、P0/P1 障害修正やセキュリティ修正は `main` に直接入れる。その場合は必要に応じて `codex/cloudflare-migration` へ forward-port し、移行ブランチの差分を腐らせない。
...
| `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` | `ClerkProvider`, Clerk browser UI | Clerk client initialization | #27 で Preview/Worker に追加 |
| `CLERK_SECRET_KEY` | `@clerk/nextjs/server` | Clerk session verification | #27 で Preview/Worker secret に追加 |
...
| Auth middleware | `src/middleware.ts` | Clerk middleware/session helper へ置換。Next/OpenNext/Cloudflare 互換性のため #27 時点では `proxy.ts` ではなく `middleware.ts` を使用 |
| Browser auth | `HomeClient.tsx`, dashboard/parent signOut | Clerk UI/API へ置換 |
| OAuth callback | `src/app/sso-callback/page.tsx`, `src/app/auth/callback/route.ts` | Clerk の Google OAuth callback は `/sso-callback`。Supabase callback は stale redirect として `/` へ退避 |
...
| Dev shortcut | Supabase anonymous sign-in | #27 で Cloudflare migration branch から廃止 |
...
| RPC | 呼び出し箇所 | 責務 | 移行先 |
|---|---|---|---|
| `complete_study_session` | `src/lib/study/completeSession.ts` | session/history/points/badges/stats/streak を app-layer service で更新 | #29 |
| `get_active_question_counts` | `src/lib/auth/data.ts` | dashboard catalog の active question count を app-layer で集計 | #29 |
| `get_child_study_status` | `src/lib/auth/data.ts` | active child の genre progress/status を app-layer で集計 | #29 |
...
- [x] Issue #25 is reviewed and closed after PR/merge.
