...
## 7. 連続学習（ストリーク）とシールド運用ルール

- 日付判定タイムゾーン: `Asia/Tokyo`
- シールド回数: 週1回（ISO週。週切り替えで1回にリセット）
- シールド発動条件: 前回学習日との差分がちょうど2日（= 1日だけ未学習）
- ストリークリセット条件: 前回学習日との差分が3日以上（= 2日以上未学習）
- `secret_comeback` は「3日以上あいてから学習を再開」で判定し、上記リセット条件と整合させる
