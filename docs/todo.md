...
- [ ] `/parent` で子ども別の獲得状況（達成率・最近獲得）を表示
- [ ] PNG命名規約を文書化（例: `badge_<family>_l<1-5>.png`, `badge_secret_<key>.png`）
- [ ] バッジ追加手順（DB定義追加 -> 画像配置 -> 動作確認）を運用ドキュメント化
- [ ] 将来のバッジ追加を見据え、マニフェスト方式（`key` と `icon_path` の対応）を定義

**完了条件**
- [ ] 新規バッジ追加時にコード変更最小で反映できる
- [ ] 保護者画面で学習の成長が把握できる

### 16-5. リリース順とガードレール
- [ ] Phase 1 完了後に内部検証（テストデータ）を実施
- [ ] Phase 2 を先行リリースして、獲得体験のフィードバックを収集
- [ ] Phase 3/4 は段階リリースし、メトリクス（継続率・再訪率）を見て調整
- [ ] 重大不具合時にバッジ表示のみを止められる feature flag を用意
