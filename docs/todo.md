...
**目的**: 子どもの継続利用と達成体験を高めるために、バッジを小さく安全にリリースしながら品質を担保する。

### 16-1. Phase 1（基盤）: データモデルと判定パイプラインを先に固める
- [x] `badge_definitions` / `child_badges` / `badge_unlock_events` / `child_streak_state` のスキーマ追加
- [x] バッジ定義のseed投入（公開4ファミリー: `streak_days` / `perfect_sessions` / `genre_explorer` / `subject_master` + secret 2）
- [x] `complete_study_session` にバッジ判定処理を統合（同一トランザクションで保存）
- [x] 冪等性（idempotency key）を維持したまま、二重解除・二重ポイント加算を防止
- [x] APIレスポンスに `unlockedBadges` を追加（UI未反映でも返却）

**完了条件**
- [x] セッション確定時にバッジ解除イベントがDBへ記録される
- [x] 同一リクエスト再送時に解除イベントと報酬ポイントが重複しない

### 16-2. Phase 2（最小体験）: 結果画面で獲得通知を出す
- [x] `/result` で当該セッションの新規獲得バッジを表示
- [x] バッジ画像（SVG）を `public/badges-arcade/svg/` 配下から表示する
- [x] 画像欠損時のフォールバック（プレースホルダー表示）を実装
- [x] バッジ獲得時の文言・報酬表示を `docs/persona.md` トーンに合わせる

**完了条件**
- [x] 新規獲得があるセッションでのみ通知カードが表示される
- [x] iPad/スマホでレイアウト崩れなく表示される

### 16-3. Phase 3（継続導線）: ダッシュボードに進捗を可視化する
- [x] `GET /api/badges/overview` を追加（現ストリーク、最近解除、次目標）
- [x] `/dashboard` はバッジ詳細カードを廃止し、学習開始導線を最優先化
- [x] `/dashboard` ヘッダーにバッジサマリ（連続日数・獲得件数）のみ表示
- [x] バッジ詳細を `/history` に統合（最近獲得・次のバッジまで・教科別達成状況）
- [x] ストリーク救済（週1シールド）運用を表示に反映
- [x] `subject_master` の進捗（国語/算数/理科/社会の特化継続）を表示に反映
- [ ] キャッシュ設計（必要なら Upstash 短TTL）を追加

**完了条件**
- [x] セッション完了後、ダッシュボード再表示で進捗が反映される
- [x] 表示値とDB集計値が一致する

### 16-4. Phase 4（保護者視点・運用）: 管理画面と運用ルールを整える
- [x] `/parent` で子ども別の獲得状況は実装しない（`/history` を主導線とする）
- [x] SVG命名規約を文書化（例: `badge_<family>_l<1-5>.svg`, `badge_secret_<key>.svg`）
- [x] バッジ追加手順（DB定義追加 -> SVG生成/配置 -> 動作確認）を運用ドキュメント化
- [x] 将来のバッジ追加を見据え、マニフェスト方式（`key` と `icon_path` の対応）を定義

**完了条件**
- [x] 新規バッジ追加時に、運用ドキュメントだけで手順が完結する
- [x] 子どもごとの学習成長は `/history` で把握できる

### 16-5. リリース順とガードレール
- [x] Phase 1 完了後に内部検証（テストデータ）を実施
- [x] Phase 2 を先行リリースして、獲得体験のフィードバックを収集
- [x] Phase 3/4 は段階リリースし、メトリクス（継続率・再訪率）を見て調整
- [x] feature flag は現時点で非採用（本アプリの運用方針に合わせる）
- [x] 重大不具合時の対応は、バッジUIの最小差分ロールバック手順を運用ドキュメントで管理
...
| 優先度 | 項目 |
|:---:|:---|
| 🔴 高 | 2. ポイント機能 / 3. サブカテゴリ / 5. 画像問題対応 / 15. Upstash Redis（短期状態・保護者スナップショット） / 16. バッジ機能（Phase 1-2） |
| 🟡 中 | 1. CI / 4. 本格ログイン / 6. 学習記録ページ / 10. 問題管理 / 14. 環境分離 / 15. Upstash Redis（ダッシュボード・問題セット） / 16. バッジ機能（Phase 3-4） |
