...
- [ ] `badge_definitions` / `child_badges` / `badge_unlock_events` / `child_streak_state` のスキーマ追加
- [ ] バッジ定義のseed投入（公開4ファミリー: `streak_days` / `perfect_sessions` / `genre_explorer` / `subject_master` + secret 2）
- [ ] `complete_study_session` にバッジ判定処理を統合（同一トランザクションで保存）
- [ ] 冪等性（idempotency key）を維持したまま、二重解除・二重ポイント加算を防止
- [ ] APIレスポンスに `unlockedBadges` を追加（UI未反映でも返却）

**完了条件**
- [ ] セッション確定時にバッジ解除イベントがDBへ記録される
- [ ] 同一リクエスト再送時に解除イベントと報酬ポイントが重複しない

### 16-2. Phase 2（最小体験）: 結果画面で獲得通知を出す
- [ ] `/result` で当該セッションの新規獲得バッジを表示
- [ ] バッジ画像（PNG）を `public/badges/` 配下から表示する
- [ ] 画像欠損時のフォールバック（プレースホルダー表示）を実装
- [ ] バッジ獲得時の文言・報酬表示を `docs/persona.md` トーンに合わせる

**完了条件**
- [ ] 新規獲得があるセッションでのみ通知カードが表示される
- [ ] iPad/スマホでレイアウト崩れなく表示される

### 16-3. Phase 3（継続導線）: ダッシュボードに進捗を可視化する
- [ ] `GET /api/badges/overview` を追加（現ストリーク、最近解除、次目標）
- [ ] `/dashboard` に「バッジ進捗」カードを追加
- [ ] ストリーク救済（週1シールド）運用を表示に反映
- [ ] `subject_master` の進捗（国語/算数/理科/社会の特化継続）を表示に反映
- [ ] キャッシュ設計（必要なら Upstash 短TTL）を追加

**完了条件**
- [ ] セッション完了後、ダッシュボード再表示で進捗が反映される
- [ ] 表示値とDB集計値が一致する
